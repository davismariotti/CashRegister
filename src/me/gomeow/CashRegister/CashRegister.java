/*
 * Author: gomeow
 * Please don't steal the source of this without permission.
 * Thank you!
 */
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label,
		...
		if (label.equalsIgnoreCase("order")) {
			...
			if (args.length == 1) {
				 ...
				 * 
				 * for(Material each : Material.class.getEnumConstants()) { int
				 * materialid = each.getId(); String pathmaterial =
				 * cap(each.toString()); this.getConfig().set("Prices." +
				 * pathmaterial.replaceAll("_", ""), materialid);
				 * 
				 * } saveConfig(); }
				 */
